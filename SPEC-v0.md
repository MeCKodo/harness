...
>
> **v0.4 相比 v0.3**：增加项目自定义 validation gates、独立 acceptance coverage、验证计划指纹与共享 gate 健康检查。`ai-harness/v0` 与 `ai-harness/v1` 都受支持；gates 只属于 v1，使只认识 v0 的旧 CLI 必须 fail closed，不能静默退化为 unit-only。任何未知标签（例如 `ai-harness/v999`）同样必须拒绝。
>
...
```yaml
spec: ai-harness/v1                  # gates 需要 v1；不使用 gates 的旧仓可继续用 v0
...
modules:
  - name: string
    ...
    gates?:  [string]                # 本模块触发的 validation gate；profile 不能绕过
...
validation:
  ...
  checksets?:                        # 命名可复用的 check 组，配合 run-checks --profile <id>
    <id>: { checks: [string] }
  gates?:                            # 项目定义的高风险验收边界；Harness 不内置前端/Electron 分类
    <id>:
      desc?: string
      checks: [string]               # gate 命中时强制追加；--profile 不能替换
      acceptance?:
        tests: [string]              # 独立的真实用户流/集成验收文件，实际文件不得与相关 owns/tests 重叠
        test_touch: required | advisory | off
  defaults?:
    no_match?: [string]              # 改动没命中任何 module.owns 时兜底跑的 checks
    always?:   [string]             # 任何改动都追加的 checks
  #  checksets/gates/defaults 里的 check 同样只能引用已声明且非 background/mutating 的 capability 动词
...
**`plan-checks`（只算不跑）**：安全调用 Git（ref 永不拼进 shell），计算 committed + staged + unstaged + untracked 的完整改动。普通 `--base` 使用 merge-base；生命周期门禁使用 SessionStart 保存的 exact commit，因而不会漏掉会话内已 commit 的文件。若 `--repo` 指向 Git worktree 内的子 target，只保留该 target 下的 entries 并去掉路径前缀，模块 glob 与证据都保持 target-relative，兄弟 package 不会混入。Git 失败、base 无效、manifest 无效都 fail closed。随后按 `modules[].owns/tests` 映射模块，选出普通 `checks`（或 `--profile <checkset>`），再强制追加受影响模块 `gates` 的 checks 与 `validation.defaults`。profile 只选择普通 checkset，不能绕过 validation gate。gate 的 acceptance 文件也参与影响面映射。计划最后输出**诚实的 gap**：
...
| `missing-test-touch` | 改了模块生产代码但没碰模块测试，或触发 gate 却没碰它独立声明的 acceptance tests | 由对应 test-touch policy 决定 |
...
| `validation-gate-invalid` | gate 未挂到模块、引用模块没有真实 production owns、required acceptance 没有真实文件，或 acceptance 与 unit/prod 边界重叠 | blocking |
...
`validation.gates` 把“必须执行什么验收”与“什么文件证明覆盖被同步维护”拆开：`checks` 是每次命中都强制执行的 capability；`acceptance.tests` 是需要随生产改动 touch 的独立用户流/集成测试 glob，普通 `modules[].tests` 不能替代它。每个 gate 都必须被至少一个 module 引用，且每个引用模块的 `owns` 必须命中仓内真实生产文件，否则 check-only gate 也可能成为永不触发的摆设，必须 fail closed。planner 会排除同时命中相关模块 `owns/tests` 的文件；`doctor`、`run-checks`、`verify` 还会共用仓库级健康检查，对 required acceptance 零文件、边界重叠、引用模块 owns 零匹配，以及通过目录 symlink 伪装的 coverage fail closed。gate 名称与边界由项目声明，Harness core 不猜 React、Electron 或具体 E2E 框架；它能证明声明的 gate command 被执行，命令是否真正消费这些 case 仍由 onboarding 对真实 scripts/config 的审计负责。

只有真实 runner 与 case 才能声明自动 gate。若项目暂时没有 E2E，onboarding 必须把具体人工旅程持久化为 `invariants[].manual: true`（并在相关模块 playbook/pitfall 中指向它），让技术债持续可见；不能只在一次聊天里报告，也不能编造 capability 或空 glob。

**`run-checks`（跑 + 留证据）**：先校验 manifest 与 gate 健康状态，再执行选中的 capability。未知、`background`、`mutating` 不是“跳过也算过”，而是 `not-verified`。状态只有：`no-change`、`verified`、`verified-with-advisories`、`verified-with-waivers`、`not-verified`。没有 lifecycle session 时，隐式 HEAD 得到的 `no-change` 不足以证明已 commit 的任务，命令会报 `manual-base-required`；手动工作流应在任务开始时记录 SHA，并传 `--base <task-start-sha>`。结果、命中的 validation gate 与验证计划指纹一起按 worktree/target/session 写入 Git admin dir 下的 `harness-kit/validation/`（文件权限 0600，保留 7 天、最多 100 条），`evidence` 可读取；同一 Git worktree 里的多个 harness target 不共享 latest 指针。计划指纹绑定 profile、affected modules、gates、selected checks、gaps 与 notes；缺少它的旧 evidence 可读取但永远不能重新变绿。
